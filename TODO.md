...
- [x] Complete package installation system integration
- [x] Create ZNode subtypes to eliminate need for `as_type` parameter
  - Ultimately, the `as_type` parameter won't usually be necessary as we'll create subtypes of ZNode that capture that information, based on parent and node type
  - This will provide automatic type narrowing and checking without requiring explicit type specification

### Node Connection Enhancements
- [x] Support multi-output znode connections with `(znode, output_index)` tuple syntax
...
- [x] Create our own placement algorithm to replace moveToGoodPosition()
...
- [x] Implement ZContext objects for shared parent and scoping control
  ...
  - Subclasses provide type safety for what kinds of nodes can be contained within other nodes
    - Example: `ZSopContext` ensures only SOP nodes can be created within geometry containers (and certain other types; the rules are complicated).
    - Example: `ZObjContext` manages object-level node creation with appropriate constraints

### Enhanced Copy Operations
- [ ] Extend `copy()` methods to support comprehensive modifications
  - Allow different inputs when copying ZNode or ZChain objects
  ...
  - Examples:
    ...
    - `chain.copy("name2", 1, znode(geo, "attribwrangle"), 3, _inputs=[alt_input])`

    The arguments can refer to nodes by name, index, or supply a new ZNode.
