...
# Install the dynamic import hook for zabob-houdini
try:
    import sys
    from pathlib import Path

    # Find zabob-houdini via PYTHONPATH
    for path_str in sys.path:
        path = Path(path_str)
        zabob_init = path / 'zabob_houdini' / '__init__.py'
        if zabob_init.exists():
            # Found zabob-houdini, now install the import hook
            from zabob_houdini.dyn_loader import install_import_hook
            install_import_hook()
            break
except Exception as e:
    # Don't fail silently - log to Houdini console
    print(f"Warning: Failed to install zabob-houdini dynamic import hook: {e}")
