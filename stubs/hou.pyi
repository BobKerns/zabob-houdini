...
from typing import Any, Optional, Union, Sequence, Iterable, overload, Type, TypeVar, Callable
...
NodePath = Union[str, 'Node']
TransformValue = Union[float, Sequence[float]]
...
class Node:
    ...
    def parent(self) -> Optional['Node']: ...  # None for root nodes
    ...
    def parmTuple(self, name: str) -> Optional['ParmTuple']: ...  # None if not a tuple parm
    ...
    def inputs(self) -> tuple[Optional['Node'], ...]: ...  # None for unconnected inputs
    def outputs(self) -> tuple['Node', ...]: ...  # Connected outputs only
    def setInput(self, input_index: int, node: Optional['Node'], output_index: int = 0) -> None: ...
    ...
    def connectedInput(self, index: int) -> Optional['Node']: ...  # None if unconnected
    def connectedInputs(self) -> list[Optional['Node']]: ...  # List with None for gaps    # Node creation and management - handle creation failures gracefully
    def createNode(self, node_type: str, name: Optional[str] = None) -> 'Node': ...  # Can raise OperationFailed
    def node(self, path: str) -> Optional['Node']: ...  # None if path doesn't exist
    ...
    def findNode(self, path: str, create_if_needed: bool = False) -> Optional['Node']: ...
...
class SopNode(Node):
    ...
    """
    def geometry(self) -> Optional['Geometry']: ...  # None if not cooked or failed
    def cook(self, force: bool = False) -> None: ...  # Can raise cooking errors
    def renderGeometry(self) -> Optional['Geometry']: ...  # Rendered geometry
    def displayGeometry(self) -> Optional['Geometry']: ...  # Display geometry
...
class RopNode(Node):
    ...
    def render(self, frame_range: Optional[tuple[int, int]] = None,
...
def findNode(path: str) -> Optional[Node]:
    """Find node by path, returns None if not found (alias for node())."""
    ...

def nodeAtPath(path: str, create_missing_dirs: bool = False) -> Optional[Node]:
    """
...
def nodeType(category: Union[str, NodeTypeCategory], name: str) -> Optional[NodeType]:
