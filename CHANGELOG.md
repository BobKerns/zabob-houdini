...
### Added
- New `ZContext` class for organizing nodes under a specific parent
  - Context manager protocol for convenient use with `with` statements
  - `node()` method creates nodes under the zcontext's parent without specifying parent
...
- New `zcontext()` function for creating ZContext instances
  - Accepts ZNode, string path, or hou.Node as parent
  - Wraps non-ZNode parents automatically for consistent interface
  - Returns ZContext that can be used as context manager
- New `ZChainBuilder` class for enhanced chain construction within contexts
  - Context manager protocol with automatic registration and layout
  - Full ZChain compatibility: `__getitem__`, `__len__`, `first`, `last`, `inputs` properties
  - Clean API without znode arguments to prevent confusion with `znode()` function
  - Seamless integration with existing ZChain/ZChainBuilder handling functions
...
- Enhanced `ZNode.copy()` with comprehensive parameter support
...
- Enhanced `ZChain.copy()` with flexible reordering and insertion capabilities
  - New `ChainCopyParam` type supporting `int`, `str`, and `ZNode` parameters
  - Index access: `chain.copy(3, 2, 1, 0)` for positional reordering
  - Name access: `chain.copy("cleanup", "input")` for name-based selection
  - ZNode insertion: `chain.copy(0, new_node, 1)` for adding new nodes
  - Mixed access: `chain.copy(0, "transform", new_node)` combining all types
- New `zmerge()` function for creating merge nodes with multiple inputs
  - Convenient shortcut: `zmerge(box, sphere, tube)` instead of `znode(parent, "merge", _input=[...])`
  - Automatic parent validation ensures all inputs have same parent
  - Supports merge node parameters: `zmerge(box, sphere, tol=0.01)`
...
### Changed
- **BREAKING**: `ZContext.chain()` method simplified to keyword-only arguments
  - Removed node arguments to eliminate API confusion with `znode()` function
  - Always returns `ZChainBuilder` for consistent behavior
  - Clean separation of concerns: `znode()` for individual nodes, `zchain()` for sequences
- `ZChain.copy()` method signature enhanced with `*copy_params: ChainCopyParam` parameter, a mapping (eg. a dict) between node names and new parameter values to apply when copying individual nodes.
...
- Enhanced test coverage for name-based access and ZNode insertion
...
- Resolved API confusion between `ctx.chain()` and `znode()` methods
- All 78 tests now passing with clean API and enhanced functionality

### Documentation
- ZChain reordering patterns section with practical examples
- Detailed `ZChain.copy()` parameter documentation and signature
- Advanced patterns covering reverse processing, partial extraction, and node duplication
- Updated examples to reflect simplified ZChainBuilder API
...
- ZNode registry using WeakKeyDictionary for node-to-instance mapping
